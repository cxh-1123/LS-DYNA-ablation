# =============================================================================
# run_v40_selected.ps1 -- Launch V4.0A dynamic ablation pilot cases
# =============================================================================
#
# Usage (from project root):
#     .\scripts\run_v40_selected.ps1 -DryRun
...
try {
    if (-not $RegistryCsv) {
        $RegistryCsv = Join-Path $projectRoot "models\v40_dynamic_ablation_pilot\v40_case_registry.csv"
    }
    if (-not (Test-Path $RegistryCsv)) {
        Write-Error "Registry CSV not found: $RegistryCsv`nRun scripts\build_v40_dynamic_ablation_pilot.py first."
        return
    }

    Write-Host "==========================================================" -ForegroundColor Cyan
    Write-Host " V4.0A LS-DYNA dynamic ablation pilot launcher" -ForegroundColor Cyan
    ...
    foreach ($r in $toRun) {
        Write-Host ("  + {0,-10} Ep={1,5} uJ  r_vap={2,8} um  ejecta_nodes={3}" -f `
            $r.name, $r.Ep_uJ, $r.vapor_radius_um, $r.ejecta_nodes)
    }
    Write-Host ""
    ...
    foreach ($r in $toRun) {
        ...
        $outDir = Join-Path $projectRoot "results\v40_dynamic_ablation_pilot\$name"
        if (-not (Test-Path $outDir)) {
            New-Item -ItemType Directory -Path $outDir | Out-Null
        }

        $kInOut = Join-Path $outDir ("v40_{0}.k" -f $name)
        Copy-Item $kAbs $kInOut -Force

        Write-Host ""
        Write-Host "[$caseIdx/$($toRun.Count)] Running V4.0A case '$name' ..." -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "V4.0A pilot sweep complete." -ForegroundColor Green
} finally {
    Pop-Location
}
